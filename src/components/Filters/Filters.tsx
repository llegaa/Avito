...
export function Filters(){
    ...
    return(
        <div className={style.container}>
            ...
            <Check text={queryParam.country ?? 'Страна'} loading={loadingCountries} error={errMsgCountries} onClick={changeParams("country")} list={countries}/>
            <Check text={queryParam.year??'Год'} loading={false} error='' onClick={changeParams("year")} list={years}/>
            <Check text={queryParam.genre??'Жанры'} loading={loadingGenres} error={errMsgGenres} onClick={changeParams("genre")} list={genres}/>
            </div>
