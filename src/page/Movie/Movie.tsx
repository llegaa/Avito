...
export function Movie() {
    ...
    return (
        <div className={style.container}>
            ...
            {!loading.movieLoading ? <div className={style.info}>
                {movie.poster && movie.poster.previewUrl && <img className={style.poster} src={movie.poster.previewUrl} alt="Постер"/>}
                <div className={style.mainInfo}>
                    <div className={style.title}>{movie.name}({movie.year})</div>
                    ...
                    {(movie && movie.persons && movie.persons.length !== 0) ?
                        <ActorList curPersonPage={curPersonPage} setCurPersonPage={setCurPersonPage} persons={movie.persons}/> : <div>Ни один актер не пострадал на съемке</div>}
                </div>
            ...
            <Title title="Сезоны"/>
            {(seasons && seasons.docs && seasons.docs.length !== 0) ? <div>
            ...
            {!loading.postersLoading ? <div>
                <Title title="Похожие фильмы"/>
                {(movie && movie.similarMovies && movie.similarMovies.length) ? <Carousel slides={movie.similarMovies.map((el) => {
